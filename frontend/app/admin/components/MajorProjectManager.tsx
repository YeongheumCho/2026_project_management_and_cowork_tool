...
import { apiFetch, type MajorProject, type UserBrief } from '../../lib/api';
...
export default function MajorProjectManager({ users }: Props) {
  ...
  function startEdit(item: MajorProject) {
    ...
    setForm({
      ...
      kickoff_date: item.kickoff_date ?? '',
    ...
    <section className="rounded-2xl border border-[#EAEAE4] bg-white p-4">
      <div className="mb-4">
        ...
        <p className="mt-1 text-small text-[#888780]">
          연 단위/계약 단위의 상위 프로젝트와 참여 인원을 관리합니다.
        </p>
      </div>
      ...
      <form onSubmit={submit} className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <DateField
            ...
            onChange={(value) => setForm((prev) => ({ ...prev, kickoff_date: value }))}
          />
          ...
          items.map((item) => (
            ...
            >
              <div>
                ...
                <p className="mt-1 text-small text-[#888780]">
                  {item.start_date ?? '-'} ~ {item.end_date ?? '-'} · 투입 {item.kickoff_date ?? '-'} · 참여 {item.members.length}명 · 중프로젝트 {item.project_count}건
                </p>
              </div>
