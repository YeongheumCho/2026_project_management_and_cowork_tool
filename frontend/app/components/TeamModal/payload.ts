...
export function buildSubProjectPayload(f: FormState) {
  return {
    ...
    verifier_id: f.verifierIds[0] ?? null,
    verifier_ids: f.verifierIds,
    reviewer_id: f.reviewerIds[0] ?? null,
    reviewer_ids: f.reviewerIds,
    inreviewer_id: f.inreviewerIds[0] ?? null,
    inreviewer_ids: f.inreviewerIds,
