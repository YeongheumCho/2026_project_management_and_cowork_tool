# 데이터베이스 초기화 & 조직도 최초 임포트

저장소를 새로 클론해 **빈 DB에서 처음 시작할 때** 쓰는 문서다.

> **이미 돌아가는 환경에서 인원만 갱신하려면 이 문서가 아니다.**
> 관리 화면에서 CSV를 올리는 방식으로 바뀌었다 → [인원 동기화 가이드](../org-sync-guide.md)
>
> 두 방법의 차이는 이렇다.
>
> | | 이 문서 (`import_org_chart.py`) | 인원 동기화 화면 |
> |---|---|---|
> | 쓰는 때 | 빈 DB에 처음 넣을 때 | 평상시 인원 갱신 |
> | 실행 위치 | 터미널 | 관리 > 인원 동기화 |
> | 퇴사자 처리 | 없음 | 비활성으로 전환 |
> | 미리보기 | 없음 | 있음 |
> | 초기 비밀번호 | `00000000` | `12345678` |
...
> [!CAUTION]
> 로컬에 남아있던 기존 데이터(프로젝트, 하위 프로젝트, 작업 로그 등)가 전부 삭제됩니다.
> 필요한 데이터는 미리 백업하세요.
